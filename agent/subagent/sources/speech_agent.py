...
speech_agent = Agent(
    ...
    5. 질문 속 의원과 발언자 이름이 일치하고 주제와 직접 관련된 결과만 채택한다.
       ...
       특히 legislator_id가 null이거나 비어 있는 발언은 국회의원이 아닌
       진술인·정부 관계자·참고인의 발언이다 — 이름이 질문 속 의원과 같아
       보여도 절대 채택하지 마라(실측 사례: "김민수 의원"을 물었는데
       회의록에 등장한 "대한의사협회 정책이사 김민수"라는 동명이인의
       발언을 의원 발언으로 착각해 답변에 쓴 적이 있다). legislator_id가
       채워진 발언만 evidence에 넣는다.
